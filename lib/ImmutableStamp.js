...
Stamp.prototype.fork = function() {
  ...
  return [new Stamp(ids[0], this.ev.clone()),
    new Stamp(ids[1], this.ev.clone())]
}

Stamp.prototype.join = function(other) {
  return new Stamp(Id.sum(this.id, other.id),
...
Stamp.prototype.event = function(other) {
  ...
  if(!ev.equals(this.ev)) {
    return new Stamp(this.id.clone(), ev.clone());
  }
  else {
    var g = Stamp.grow(this.id, this.ev)
    return new Stamp(this.id.clone(), g[0]);
  }
}
