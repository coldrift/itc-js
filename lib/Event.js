...
Event.prototype.norm = function() {
  if(Object.getPrototypeOf(this.left) === EventLeaf.prototype
    && Object.getPrototypeOf(this.right) === EventLeaf.prototype
    && this.left.value === this.right.value)
  {
    return new EventLeaf(this.value + this.left.value)
  }

  var l = this.left.norm()
  var r = this.right.norm()
  var m = min(l.min(), r.min())
  return new Event(this.value + m, l.sink(m), r.sink(m))
}
